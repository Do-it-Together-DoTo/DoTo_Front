...
const ChattingRoomPage = () => {
  ...
  return (
    <main className="w-[54rem] h-[45rem] flex  flex-col gap-6 bg-Light_Layout-100 dark:bg-Dark_Layout-300 items-center py-14 ">
      <section className="flex flex-col w-[33.75rem] py-8 px-9 bg-Light_Layout-400 gap-6 rounded-2xl">
        ...
        <div className="flex flex-col w-[29rem] h-[30rem] px-5 py-6 bg-Light_Layout-100 rounded-2xl justify-between">
          <div className="flex flex-col w-full gap-5 max-h-70 py-3 overflow-y-scroll">
            {messages.map((message) => (
              <ChattingMessage message={message} />
            ))}
          </div>
          <div className="pl-2 py-3 bg-Light_Layout-400 rounded-[0.625rem] ">
            <textarea rows={4} className="w-full bg-Light_Layout-400 resize-none outline-none" />
          </div>
        </div>
      </section>
    </main>
