...
const FriendItem = ({ member, isFriend }: IFriendItemProps) => {
  return (
    <li className="flex gap-4 items-center">
      ...
      <span className="text-Light_CategoryText_Icon_Contents text-base">{member.nickname}</span>
      {isFriend ? (
        <>
          <button className="w-[4.375rem] h-[1.5625rem] bg-Light_CategoryText_Icon_Contents text-Light_Layout-400 text-[0.625rem] rounded-[0.6875rem] ">
            친구삭제
          </button>
          <button className="w-[4.375rem] h-[1.5625rem] bg-Light_Layout-300 text-[0.625rem] rounded-[0.6875rem] text-Light_Text_AboutMe">
            취소
          </button>
        </>
      ) : (
