...
const OpenBettingModal = ({ betting, onClose }: BettingDetailModalProps) => {
  ...
  async function fetchJoinBetting() {
    try {
      const response = await joinBetting(betting.bettingId, {
        cost: cost,
        prediction: isDo,
      });
      ...
      >
        ...
        <div className="flex flex-col items-center gap-1">
          <div className="flex items-center">
            <CheckBox className="flex [&>input]:hidden text-Button font-bold text-xl" checked={isDo} onChange={setIsDo}>
              <p>{isDo ? '한다' : '안한다'} </p>
            </CheckBox>
            에
            <CoinIcon width="2rem" height="2rem" />
            ...
            <input
              ...
              className="border-b px-4 w-20 focus:outline-none"
            />
            를 베팅할게요.
          </div>
          <p>{isValid && '금액이 부족합니다.'}</p>
        </div>
...
export default OpenBettingModal;
