...
export const getJoinChatting = async () => {
  return await axiosInstance.get(END_POINT.MY_CHATTING);
};

//참여 중인 채팅방 조회
export const getChatting = async (chatRoomId: number) => {
  return await axiosInstance.get(END_POINT.CHATTING(chatRoomId));
};

//채팅방 참여
export const joinChatting = async (chatRoomId: number) => {
  return await axiosInstance.post(END_POINT.CHATTING(chatRoomId));
};
